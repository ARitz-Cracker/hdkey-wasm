...
var MASTER_SECRET = BufferLib.stringToBuffer('Bitcoin seed');
...
HDKey.prototype.deriveChild = function (index) {
  ...
  var indexBuffer = new Uint8Array(4);
  ...
  if (isHardened) { // Hardened child
    ...
    var zb = BufferLib.newBuffer(1)
    pk = BufferLib.concat([zb, pk])

    // data = 0x00 || ser256(kpar) || ser32(index)
    data = BufferLib.concat([pk, indexBuffer])
  } else { // Normal child
    // data = serP(point(kpar)) || ser32(index)
    //      = serP(Kpar) || ser32(index)
    data = BufferLib.concat([this._publicKey, indexBuffer])
  }
